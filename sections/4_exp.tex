...
We compare \model{} against baselines spanning multiple model families that differ in how, and whether, they incorporate text into visual features (see supplement
% \cref{sec:related_work} for an extended discussion and 
% \cref{app:baseline_extraction}
for feature-extraction details):
...
(e.g., a fruit bowl in a kitchen; a backpack in the park; see supplement for details).
...
MAE barely exceeds random chance and DINOv2, despite its object-centric representations, achieves only 43\% acc@1.
...
To verify that steerability is genuinely text-driven rather than an artifact of training, we condition each model on a random (incorrect) object class.
...
The \texttt{[CLS]}-to-patch attention scores in the final self-attention block highlight how global attention is redirected to prompt-specific regions (\cref{fig:heatmap_teaser}). 
...
When prompted with ``potted plant'', it distributes attention among the class occurrences in the bottom-right image. 
Additional qualitative examples are in the supplement.
...
This knob is especially impactful when applying \model{} to SigLIP, where feature quality degrades sharply but can be recovered in large part by setting $\alpha_\ell' = \alpha_\ell * 0.6$ while retaining meaningful steerability as shown in the orange curve of \cref{fig:gate_factor}.
...
We explore the role of text in forming high-fidelity visual representations.
% In this section, we systematically explore how text steers our visual representation.
We choose Personal Object Discrimination Suite (PODS)~\cite{sundaram2024personalized_pods}, a benchmark evaluating the formation of instance-aware feature spaces via the ability of models to recognize particular objects (e.g., \textit{your} mug vs. all other mugs).
...
\textbf{(2)} Conditioning \colorbox{\methodcolor}{\model{}} on \texttt{``animal''} merges animal classes into one cluster while separating non-animals.
...
Conditioning on \texttt{``eye''} (Figure~\ref{fig:tsne_semantic}, (4)) produces two macro clusters: animate classes that possess eyes versus inanimate objects that do not.
...
Here, models are conditioned on prompts in the style of \texttt{``the anomaly in the <object>''} and anomaly maps are derived by upsampling the heatmaps produced by the learned linear segmentation head. We compare \model{} to other segmentation models and dedicated AS methods using \textit{Per-Region-Overlap} (PRO).
More details and extended results are provided in the supplement. 
