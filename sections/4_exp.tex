...
\MT{I'm wondering whether it would make sense to show some visual examples along with this scatter plot? current bar plot on the right can be a separate figure perhaps} \JR{on it ...} \MG{write about subclustering}
}
...
In contrast, \model{} achieves 94.3\% retrieval accuracy, confirming that text conditioning shifts the global representation from the scene level (``kitchen'') to the queried concept (``fruit bowl''). \MG{consistent with Fig2a}
...
We observe an optimal operating point at a scaling factor of 0.6: the model preserves the foundational feature quality of DINOv2 while unlocking high text steerability, outperforming both standard encoders and models like SAM3 and GroundingDINO.
...
\MT{we should use a different color to highlight our approach, also call it SHIFT. all text can be bigger and more readable, bars can be thinner} \JR{on it ...}
}
\label{fig:pods}
\end{figure}
...
The previous sections established that SHIFT steers global features with text, routes dense attention to queried concepts, and preserves representation quality.
We now ask: does this combination enable generalization to domains unseen during training?
We evaluate on two progressively harder out-of-distribution settings: personalized object discrimination (natural images of novel object instances) and industrial anomaly detection (factory inspection imagery absent from all training data).

\subsubsection{Personalized Object Discrimination}

Section~\ref{subsec:text_density_exp} showed that text specificity controls feature granularity on the PODS benchmark, an out-of-distribution task involving personalized object instances in the wild.
A surprising result emerges from that analysis: with descriptive text prompts, SHIFT not only adapts to this OOD domain zero-shot but outperforms DINOv2 supervised fine-tuned on labeled target data.
...
CLIP and SigLIP with late text fusion also fail on this task, confirming that OOD generalization requires both a rich frozen vision encoder and early text conditioning.
...
Analogous to how prompting/in-context learning adapts large language models to novel tasks without retraining, descriptive text adapts visual representations to novel domains without fine-tuning, leveraging the representational depth that unimodal pretraining already provides.

\subsubsection{Anomaly Detection: Extreme OOD Domain Transfer}

Having established OOD generalization on natural images, we escalate the domain gap: industrial anomaly detection requires localizing subtle manufacturing defects in factory inspection imagery, a domain entirely absent from our training data (RefCOCO, Visual Genome, Mapillary Vistas).
...
SHIFT again matches dedicated methods on this extreme OOD setting.
