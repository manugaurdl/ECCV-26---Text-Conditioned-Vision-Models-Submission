...
In the previous section, we demonstrated that text conditioning can effectively steers global semantics. 
...
For that, we construct a benchmark by stitching together four images from PASCAL-VOC \cite{pascal_voc} into a single $2\times2$ mosaic, reducing the saliency of each primary subject.
By measuring the area under the precision-recall-curve (AUPRC) of the \texttt{[CLS]}$\rightarrow$patch attention scores in the ultimate layer, we can quantify how effectively global attention is redirected to prompt-specific regions\footnote{Tokens consistently acting as attention sinks are ignored (cf. \cite{darcet2023vitneedreg}).}. 

\subsubsection{Qualitative Analysis}
As revealed by Figure \ref{fig:steer_attn}, DINOv2's attention map focuses primarily on the prominent ``pony'' and ``airplane'' entities, confirming the saliency bias hypothesized in Section~\ref{sec:intro}.
...
Additional qualitative examples appear in the Appendix. \JR{need to create and link apdx section.}
...
We have shown that query specificity dictates the level of semantic abstraction (\cref{subsec:text_density_exp}).
Here we visualize how text conditioning reshapes embedding space topology via UMAP.
DINOv2 imposes a fixed topology dictated by saliency, clustering images by their dominant foreground object.
\model{} dynamically reorganizes this topology according to the text query.
We demonstrate two complementary modes: steering along the \textit{semantic hierarchy} and steering by \textit{compositional attributes}.
% --- Old intro (commented out) ---
...
% In this section, we analyze U-MAP visualizations to better understand the embedding space topology,  consequently \model{}'s ability to steer semantics.
% U-MAP visu reveal how text conditioning reshapes the embedding space topology.
% Standard ViT's like DINOv2 exhibit a fixed topology dictated by saliency bias, clustering images by the dominant foreground object.  
% Our method on the other hand \model{} dynamically reshapes this topology based on the input text query.
% We explore this by visualizing \textit{Semantic Steering} and \textit{Object Composition Steering}. 
% \MG{to do MG: compress}
...
\paragraph{Setup.} We select images spanning nine PASCAL-VOC classes: bird, bus, car, cat, dog, horse, motorbike, person, and train.

\paragraph{Analysis.}
As shown in Figure~\ref{fig:tsne_semantic} (panel~1), DINOv2 embeddings cluster rigidly by object class.
Conditioning on \texttt{``animal''} (panel~2) merges all animal classes (bird, cat, dog, horse) into a single macro-cluster while pushing non-animal classes apart.
Crucially, fine-grained structure is preserved within the animal cluster: dogs and cats remain closer neighbors than birds, confirming that text controls clustering granularity without destroying object-level semantics.
...
This confirms that text can steer clustering at multiple levels of the semantic hierarchy simultaneously.
...
Semantic categories are not the only axis along which text can steer features.
We also explore steering by compositional attributes i.e shared sub-object properties. This defines an grouping principle orthogonal to semantic categories.

Conditioning on \texttt{``eye''} (Figure~\ref{fig:tsne_semantic}, panel~4) produces two clusters: animate classes that possess eyes (bird, cat, dog, horse, person) versus inanimate objects (bus, car, motorbike, train).
...
{\color{gray} We derive the anomaly map by upsampling the continuous heatmap produced by the learned linear segmentation head.
...
On VisA, \model{} ties FADE on $\text{ROC}_P$ (91.5) and reaches 77.5 PRO, indicating robust transfer to a harder, more diverse inspection setting.}
\MG{move feature extraction and table to Supplementary? 3 columns: a) and b) qualitative results, c) quantify results iwth line plot -  one metric ROCp - 2 lines on a plot for MVtech and VisA?}
