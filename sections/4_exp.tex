...
\vspace{-3mm}
\caption{\textbf{Text enables targeted attention.} Attention maps on a four-image mosaic demonstrate that text conditioning with \colorbox{\methodcolor}{\model{}} redirects self-attention to the queried concept whereas \colorbox{visionblue}{DINOv2} attends to the most prominent objects. % (``pony'' and ``airplane'').
}
\vspace{-4mm}
...
Here, models are conditioned on an ensemble of ten prompts in the style of \texttt{``the anomaly in the <object>''} and anomaly maps are derived by upsampling the heatmap produced by the learned linear segmentation head. We compare to other segmentation models as well as dedicated AS methods using \textit{Per-Region-Overlap} (PRO).
...
These results reinforce the pattern of text-driven steering unlocking capabilities already present in the frozen ViT and transferring them to domains beyond the training distribution. \MG{ensemble part unclear + add examples}
