...
In this section, we empirically validate various qualities of our learned steerable representations and show applications to diverse tasks such as personalized object discrimination or anomaly detection.
...
\MT{say something about model size here to indicate why not QwenVL3 7B or so}
% \item
(4)~\textbf{Open-vocabulary localization}: SAM3~\cite{carion2025_sam3} and GroundingDINO~\cite{liu2023groundingdino}; we evaluate the 
...
Here, both the query as well as the gallery images are extracted by conditioning on the name of the query object $\Omega$ (e.g., ``the fruit bowl'').% to prevent ground-truth leakage through the text embeddings. 
...
FLAIR exhibits a moderate drop ($-36.1\%$), while \model{} drops by $-50.8\%$, indicating stronger text-dependence. \JR{These are percentage points and not relative percent, right?}
...
By considering the \texttt{[CLS]}$\rightarrow$patch attention scores in the ultimate self-attention block, we can highlight how global attention is redirected to prompt-specific regions\footnote{Tokens consistently acting as attention sinks are ignored (cf. \cite{darcet2023vitneedreg}).}. 
...
In contrast, \model{} conditioned on ``person'' routes attention to the faintly visible person in the top-left. When prompted with ``potted plant'', it distributes attention among the instances of this within the bottom-right image. 
Additional qualitative examples appear in the Appendix. \JR{need to create and link apdx section.}

\subsubsection{Quantitative Evaluation.}
This effect is further quantified by measuring the area under the precision-recall-curve (PR-AUC) of the continuous attention heatmaps and the ground-truth segmentation masks from PASCAL-VOC for each type of object appearing in the mosaic instance.
Whereas DINOv2 cannot be actively steered and opaquely focuses on mostly prominent objects, resulting in a low PR-AUC of 14.3\%, the textual steering allows \model{} to achieve a substantially higher score of 42.5\% across 363 mosaics, indicating increased spatial focus on the areas of interest during global aggregation.
\MT{why not report AP which is more standard for segmentation?} \JR{Re: our heatmaps are continuious; PR-AUC allows for threshold-less assessment}
...
While previous sections revealed high steerability of certain methods, this property is only useful if it preserves the downstream transfer that makes visual representations valuable in the first place.
...
To assess this trade-off, we contrast performance on the CORE steerability benchmark (cf. \ref{subsec:cond_ret_exp}) with vision-centric downstream tasks intended to measure representational quality. 
In addition to training linear probes on global features across three fine-grained classification datasets (ImageWoof \cite{}, Waterbirds \cite{}, StanfordCars \cite{stanford_cars}), binary object-of-interest segmentation on ADE20k \cite{} gauges the dense encoding performance. 
Here, promptable models are conditioned on the superclass (e.g., ``dog'' for ImageWoof) and the object name, respectively. 
...
(2) \textbf{MLLMs} perform reasonably on classification but falter on dense tasks and require billions of parameters for moderate steerability.
...
As training progresses, the gates learn slowly open, learning to incorporate text information in the visual features as illustrated in ~\cref{fig:gate_factor_train_iter} \JR{@MG this figure doesn't exist}.
...
\YA{I don't think it's clear what models are ours vs not. also bigger fontsizes, and maybe just further reduce the retrieval y-axis range to 40-90?; the right plot should somehow highlight "the query" a bit more, and show that these are OUR results? }}
\label{fig:pods}
\end{figure}

Section~\ref{subsec:cond_ret_exp} found that the text you condition on has a profound impact on the resultant features with conditioning on a random object class halving retrieval performance.
In this section, we systematically explore the role of text in forming high-fidelity visual representations.
% In this section, we systematically explore how text steers our visual representation.

We do this using the Personal Object Discrimination Suite (PODS) \cite{sundaram2024personalized_pods}, a benchmark evaluating the formation of instance-aware feature spaces via the ability of models to recognizing specific instances of objects (e.g., ``your'' mug vs. all other mugs). Given a small set of reference images, PODS computes cosine similarities on frozen global features and performs (i) one-vs-all instance classification as well as (ii) similarity-based retrieval over a set of test images. % under distribution shifts.
Following \cite{sundaram2024personalized_pods}, we report PR-AUC (i) and NDCG scores (ii), respectively. 


\subsubsection{From Coarse to Fine-Grained Steering.}
We vary the amount of detail provided to \model{} via the text prompt from coarse supercategories (e.g., ``mug'', ``shoe''), over object names (e.g., ``white ECCV mug''), to comprehensive descriptions of each instance's visual appearance. 
As shown in Figure~\ref{fig:pods}, the semantics of the steered representations are highly sensitive to the prompt specificity.
