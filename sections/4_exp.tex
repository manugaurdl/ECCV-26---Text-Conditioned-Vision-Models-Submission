...
We observe an optimal operating point at a scaling factor $\lambda = 0.6$: the model preserves the foundational feature quality of DINOv2 while unlocking high text steerability, outperforming both standard encoders and models like SAM3 and GroundingDINO. \MG{knob helps with siglip specifically}
...
\begin{subfigure}[b]{0.4\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/mosaic_gate_factor.png}
\label{fig:mosaic_gate_factor}
\end{subfigure}
\caption{
% \textbf{Gate scaling controls the quality-steerability tradeoff.} 
Mosaic AUC increases with gate scaling, reaching an optimal tradeoff at 0.6.
\MT{dino is a lame comparison, can we compare to groundingdino? i guess we are worse?} \MG{We can only evaluate Dino right now since we compute attention from CLS token}
}
...
\item \textbf{Fine-grained (Descriptive):} Highly descriptive prompts (generated by human or MLLMs) act as precise steering mechanism, yielding the largest gains and enabling zero-shot instance-level discrimination that surpasses supervised fine-tuned DINOv2.
