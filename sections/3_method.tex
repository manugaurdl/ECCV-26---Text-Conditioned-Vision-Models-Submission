...
Our method adds lightweight vision-to-language cross-attention layers within pretrained ViT blocks and applies a patch-level segmentation proxy objective to fuse prompt cues into patch tokens. \YA{add:We train this with existing referential object localisation datasets of X~M images??}
...
For an image $X_v \in \mathbb{R}^{H \times W \times 3}$, a ViT produces a sequence of $N$ patch tokens $Z_v \in \mathbb{R}^{N \times d}$ and \YA{optionally} a  \texttt{[CLS]} token, where $d$ is the embedding dimension.
...
We adopt a frozen, pretrained text encoder (RoBERTa-Large~\cite{liu2019roberta}) to produce token-level embeddings from the input conditioning prompt.
...
\YA{output of roberta is variable length? so then need to explain that MLP is applied token-wise}

\colorbox{textred}{\textbf{C. Multimodal adapter}}
\JR{make starting with '.' consistent}
is a trainable two-layer MLP that projects the normalized text embeddings $Z_t$ into visual-aligned tokens $H_t \in \mathbb{R}^{L \times d}$, matching the ViT's embedding dimension $d$.
...
{\textbf{D. Gated cross-attention layers.}}
...
As training progresses, the gates learn to open, allowing the model to smoothly incorporate text conditioning. \YA{I think something like this would be nice to add:
...
\end{equation}
and $\mathrm{sech}^2(0)=1$, yielding
\(
\left.\frac{\partial Z_v^{(\ell+1)}}{\partial \alpha_\ell}\right|_{\alpha_\ell=0}
=
\mathrm{FFN}(\hat{Z}_v^{(\ell)}).
\)
Thus $\alpha_\ell$ can move away from zero during optimization, gradually activating the conditioning pathway.
}
We insert these CA layers in every other Transformer encoder block (e.g., 6 layers across 12 blocks in ViT-B).

% \MT{TODO}
% We insert these cross-attention layers at every third layer 
% (i.e.~$\ell = {2, 5, 8, 11}$ and 0-indexed for the 12-layer ViT-B).
% starting from 2 (i.e., blocks 2, 5, 8, 11 for the 12-block ViT-B).
...
\label{sec:objective}
\YA{first sentence should be that we design a pretext task and that task needs to somehow motivate text-integration. therefore we do referntial segmentation, but with a twist: we do it only at patch-level, as this is significantly cheaper and faster (no decoder needed). and we do it with gaussian... [IS THIS STILL WHATR WE DO?]}
We train \model{} on referential segmentation at \YA{the original pretrained } ViT's patch resolution rather than at pixel level. 
...
A linear classifier head maps each patch representation $z_i \in \mathbb{R}^{d}$ to a probability $p_i$ through softmax.
