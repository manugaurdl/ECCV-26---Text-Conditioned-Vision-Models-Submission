...
\caption{%
Architecture ablations on DINOv2 ViT-B/14.
\model{} combines three principal design choices; each ablation row toggles exactly one (marked \xmark).
}
\vspace{-5mm}
\label{tab:arch_ablation}
\begin{tabular}{@{}ccc
...
\bottomrule
\end{tabular}
% \begin{tablenotes}[flushleft]\footnotesizeTop gray row: frozen DINOv2 baseline (no adapter \JR{in Sec. 3, we call the MLP adapter; might be better to use other term here.}).
...
\xmark\ in MLP Proj.\ means single linear layer. % replacing the two-layer MLP.
Vanilla DINOv2 baseline (top row, gray) does not consider text.
% \end{tablenotes}
\end{threeparttable}
\vspace{-6mm}
% \end{table}
